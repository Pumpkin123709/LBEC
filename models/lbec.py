...
def make_model(args):
    return LBEC(args).cuda()


class SeparableConv2d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, bias=False):
        super(SeparableConv2d, self).__init__()
        self.conv1 = torch.nn.Conv2d(in_channels, in_channels, kernel_size, stride, padding, dilation, groups=in_channels)
...
class KernelEstimation(torch.nn.Module):
    def __init__(self, kernel_size):
        ...
        def BasicM(input_channel, output_channel):
                    return torch.nn.Sequential(
                        SeparableConv2d(in_channels=input_channel, out_channels=output_channel, kernel_size=3, stride=1, padding=1),
                        torch.nn.ReLU(inplace=False),
                        SeparableConv2d(in_channels=output_channel, out_channels=output_channel, kernel_size=3, stride=1, padding=1),
                        torch.nn.ReLU(inplace=False),
                        SeparableConv2d(in_channels=output_channel, out_channels=output_channel, kernel_size=3, stride=1, padding=1),
                        torch.nn.ReLU(inplace=False)
                    )
        ...
        def UpsampleM(channel):
                    return torch.nn.Sequential(
                        torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                        SeparableConv2d(in_channels=channel, out_channels=channel, kernel_size=3, stride=1, padding=1),
                        torch.nn.ReLU(inplace=False)
                    )
        ...
        def Se(channel):
            return torch.nn.Sequential(
                ...
                torch.nn.Conv2d(channel,channel//16,kernel_size=1),
                torch.nn.ReLU(),
                torch.nn.Conv2d(channel//16,channel,kernel_size=1),
                torch.nn.Sigmoid()
            )
        ...
        self.moduleConv5 = BasicM(256, 512)
        self.modulePool5 = torch.nn.AvgPool2d(kernel_size=2, stride=2)
        self.se4 = Se(512)
        self.se4a = Se(512)

        self.moduleDeconv6 = BasicM(1024,512)
        self.moduleUpsample6 = UpsampleM(512)

        self.moduleDeconv5 = BasicM(512, 512)
        self.moduleUpsample5 = UpsampleM(512)

        self.moduleDeconv4 = BasicM(512, 256)
        self.moduleUpsample4 = UpsampleM(256)
    ...
    def forward(self, rfield0, rfield2):
        ...
        tensorJiona = torch.cat([rfield2, rfield0], 1)

        tensorConv1 = self.moduleConv1(tensorJoin)
        tensorPool1 = self.modulePool1(tensorConv1)
        tensorConv1a = self.moduleConv1(tensorJiona)
        tensorPool1a = self.modulePool1(tensorConv1a)

        tensorConv2 = self.moduleConv2(tensorPool1)
        tensorPool2 = self.modulePool2(tensorConv2)
        se2 = self.se1(tensorPool2)
        tensorConv2a = self.moduleConv2(tensorPool1a)
        tensorPool2a = self.modulePool2(tensorConv2a)
        se2a = self.se1a(tensorPool2a)

        tensorConv3 = self.moduleConv3(tensorPool2)
        tensorPool3 = self.modulePool3(tensorConv3)
        se3 = self.se2(tensorPool3)
        tensorConv3a = self.moduleConv3(tensorPool2a)
        tensorPool3a = self.modulePool3(tensorConv3a)
        se3a = self.se2a(tensorPool3a)

        tensorConv4 = self.moduleConv4(tensorPool3)
        tensorPool4 = self.modulePool4(tensorConv4)
        se4 = self.se3(tensorPool4)
        tensorConv4a = self.moduleConv4(tensorPool3a)
        tensorPool4a = self.modulePool4(tensorConv4a)
        se4a = self.se3(tensorPool4a)

        tensorConv5 = self.moduleConv5(tensorPool4)
        tensorPool5 = self.modulePool5(tensorConv5)
        se5 = self.se4(tensorPool5)
        tensorConv5a = self.moduleConv5(tensorPool4a)
        tensorPool5a = self.modulePool5(tensorConv5a)
        se5a = self.se4(tensorPool5a)

        tensorConv6 = torch.cat([tensorPool5, tensorPool5a], dim=1)

        tensorDeconv6 = self.moduleDeconv6(tensorConv6)
        tensorPool6 = self.moduleUpsample6(tensorDeconv6)

        tensorDeconv5 = self.moduleDeconv5(tensorPool6)

        # tensorUpsample5 = self.moduleUpsample5(tensorDeconv5)

        tensorCombine = tensorDeconv5 + se5*tensorConv5 + se5a*tensorConv5a

        tensorDeconv4 = self.moduleDeconv4(tensorCombine)
        tensorUpsample4 = self.moduleUpsample4(tensorDeconv4)

        tensorCombine = tensorUpsample4 + se4*tensorConv4 + se4a*tensorConv4a

        tensorDeconv3 = self.moduleDeconv3(tensorCombine)
        tensorUpsample3 = self.moduleUpsample3(tensorDeconv3)

        tensorCombine = tensorUpsample3 + se3*tensorConv3 + se3a*tensorConv3a

        tensorDeconv2 = self.moduleDeconv2(tensorCombine)
        tensorUpsample2 = self.moduleUpsample2(tensorDeconv2)

        tensorCombine = tensorUpsample2 + se2*tensorConv2 + se2a*tensorConv2a
...
class LBEC(torch.nn.Module):
    def __init__(self, args):
        super(LBEC, self).__init__()
        self.args = args
